...
namespace pr {
  ...
  typedef Eigen::Matrix<float, 1,2+400+350> VectorDescriptor; // Dimension descriptor = position of the landmark + number of angle + number of range in Radar scan
  ...
  inline Eigen::Matrix3f v2tEuler2D(const Eigen::Vector3f& v){
    Eigen::Matrix3f T;
    T.setIdentity();
    T.block<2,2>(0,0)=Rtheta(v[2]);
    T.block<2,1>(0,2)=v.head<2>();
    return T;
  }

  inline Eigen::Vector3f t2vEuler2D(const Eigen::Isometry2f& T){
    Eigen::Vector3f v;
    v.head<2>()=T.translation();
    v(2) = atan2(T.linear()(1,0),T.linear()(0,0));
    return v;
  ...
  inline Eigen::Vector3f t2vEuler2D(const Eigen::Matrix3f& T){
    Eigen::Vector3f v;
    v.head<2>()=T.block<2,1>(0,2);
    v(2) = atan2(T(1,0),T(0,0));
    return v;
  }
